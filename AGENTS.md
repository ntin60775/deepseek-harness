...
This checkout is a fork of `deepseek-ai/deepseek-harness`. Fork-local surfaces: the `packages/context/agent-rules` plugin, its bundle/tsconfig/lock/snapshot wiring, generated-catalog rows (en+zh), the `Fork-local`-marked dev icon patch in `apps/desktop/src/main.ts`, and GitHub-side workflow disables. Incorporate upstream with `scripts/fork-merge-ritual.sh` (never pushes). Deploy the Desktop app from this checkout per [running-the-desktop-app](docs/cookbook/running-the-desktop-app.md). The fork targets Linux x64 only: keep `e2e.yml` (338747924), `ci-master.yml` (339387441), and `sandbox.yml` (338747938) disabled GitHub-side via `gh api repos/ntin60775/deepseek-harness/actions/workflows/<id>/disable -X PUT` — any edit to their files re-enables them, and the ritual re-checks ([ritual note](.agents/notes/implemented/process/2026-09-08-fork-merge-ritual.md)). On any Linux host run the operator-local `~/.local/bin/dsh-test` (template in the linked note) instead of `pnpm run test`: it applies `umask 022` (the spill sweep trust check), retries flakes, and treats proven upstream-owned host-drift files as expected failures ([validation lanes](.agents/notes/implemented/process/2026-09-10-fork-linux-x64-validation-lanes.md)); never patch those files here.
