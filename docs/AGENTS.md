...
- **Chinese documentation is not maintained in this fork.** `.zh.md` files arrive through upstream merges only; fork-local documents are English-only (exclusions in `scripts/translation-pairing.manifest.json`; see root [AGENTS.md](../AGENTS.md#fork-maintenance)).
